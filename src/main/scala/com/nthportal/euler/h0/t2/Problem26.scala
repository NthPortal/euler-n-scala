...
object Problem26 extends ProjectEulerProblem {
  ...
  private def longDivide(dividend: Int, divisor: Int): DecimalNumber = {
    ...
    val (nonRepeating, repeating) = doLongDivide(dividend * 10, divisor)
  ...
  private def doLongDivide(dividend: Int,
                           divisor: Int,
                           index: Int = 0,
                           currentSolution: StringBuilder = mutable.StringBuilder.newBuilder,
                           ops: mutable.Map[(Int, Int), Int] = mutable.HashMap()): (String, String) = {
